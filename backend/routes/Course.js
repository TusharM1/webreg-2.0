...
router.post("/create", async (req, res) => {
	if (req.body) {
		...
		if (schoolNumber && departmentNumber && courseNumber && courseName && numberOfCredits) {
			const confirmation = await createCourse(
				schoolNumber,
				departmentNumber,
				courseNumber,
				courseName,
			...
			if (!confirmation && confirmation.error) {
				res.json({status: "failure", message: "Unsuccessfully created course, error: " + JSON.stringify(confirmation.error)});
				return;
			}

			console.log(`Created course ${courseName} ` +
				`(${courseNumbersToString(schoolNumber, departmentNumber, courseNumber)}, ` +
				`${numberOfCredits})`)
...
router.post("/edit", async (req, res) => {
	if (req.body) {
		...
		if (schoolNumber && departmentNumber && courseNumber && courseName && numberOfCredits) {
			const confirmation = await editCourse(
				schoolNumber,
				departmentNumber,
				courseNumber,
				courseName,
			...
			if (!confirmation && confirmation.error) {
				res.json({status: "failure", message: "Unsuccessfully edited course, error: " + JSON.stringify(confirmation.error)});
				return;
			}

			console.log(`Edited course ${courseName} ` +
				`(${courseNumbersToString(schoolNumber, departmentNumber, courseNumber)}, ` +
				`${numberOfCredits})`)
...
router.post("/remove", async (req, res) => {
	if (req.body) {
		...
		if (schoolNumber && departmentNumber && courseNumber) {
			const confirmation = await removeCourse(
				schoolNumber,
				departmentNumber,
				courseNumber
			);
			if (!confirmation && confirmation.error) {
				res.json({status: "failure", message: "Unsuccessfully removed course, error: " + JSON.stringify(confirmation.error)});
				return;
			}
