...
export function AdminUI() {
	...
	const createCourse = async (coursestring, schoolNum, departmentNum, courseNum, namE, credits) => {
		axios.post("http://localhost:3001/createCourse", {
			...
			console.log("Hey this is line 23 of AdminUI.js");
		});
	};

	const removeCourse = async (coursestring) => {
		axios.post("http://localhost:3001/removeCourse", {
			token: data.token, coursestring: coursestring
		}).then((response) => {
			alert(JSON.stringify(response.data));
		}).catch(() => {
			console.log("Hey this is line 36 of AdminUI.js");
		});
	};

	const updateCourse = async (coursestring, namE, credits) => {
		axios.post("http://localhost:3001/updateCourse", {
			token: data.token, coursestring: coursestring, namE: namE, credits: credits
		}).then((response) => {
			alert(JSON.stringify(response.data));
		}).catch(() => {
			console.log("Hey this is line 47 of AdminUI.js");
		});
	};
	...
	return (<Container fluid>
			<form onSubmit={async (values, actions) => {
				...
				<button type="submit" id="createClass">CreateClass</button>
			</form>
			<form onSubmit={async (values, actions) => {
				...
				<button type="submit" id="removeClass">Remove Class</button>
			</form>
			<form onSubmit={async (values, actions) => {
				...
				<button type="submit" id="updateClass">Update Class</button>
			</form>
		</Container>);
}